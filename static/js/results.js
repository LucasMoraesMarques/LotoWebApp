function handleDrawsSelect() {
  // Get current lottery draws numbers
  $.ajax({
    type: "GET",
    ...
    success: function (response) {
      ...
      if (collections.length == 0) {
        ...
        fireToast(
          "Não há coleções para essa loteria",
          "Crie coleções",
          "warning"
        );
      } else {
...
function handleResultsGeneration(e) {
  // Submit collection to check in a given draw and receive a file with points by game
  e.preventDefault();
  ...
  $.ajax({
    ...
    success: function (response) {
      ...
      $("#value-paid").html(
        total_balance["Valor Gasto"].toLocaleString("pt-br", currency)
      );
      $("#prizes-value").html(
        total_balance["Premiacao"].toLocaleString("pt-br", currency)
      );
      $("#balance").html(
        total_balance["Saldo"].toLocaleString("pt-br", currency)
      );
...
function handleSendResults() {
  var send_by = this.id;
  fireModal(
    `Você realmente deseja enviar os resultados selecionados por ${send_by}?`,
    "Tem certeza ?",
    "warning"
  ).then((answer) => {
    if (answer.isConfirmed) {
      ...
      if (rows[0].length != 0) {
        ...
        fireToast(
          "Nenhum resultado foi selecionado para envio. Clique nos botões na primeira coluna para selecionar",
          "Atenção",
          "warning"
        );
      }
...
function handleExportResults() {
  fireModal(
    `Você realmente deseja EXPORTAR os resultados selecionados?`,
    "Tem certeza ?",
    "warning"
  ).then((answer) => {
    if (answer.isConfirmed) {
...
function handleExportResultGames() {
  fireModal(
    `Você realmente deseja EXPORTAR os jogos selecionados?`,
    "Tem certeza ?",
    "warning"
  ).then((answer) => {
    if (answer.isConfirmed) {
...
function handleCreateGamesetFromSelected() {
  ...
  SwalModal.fire({
    ...
    if (result.isConfirmed) {
      ...
      form.append(
        `<input type="hidden" name="gameset-name" value="${result.value.gameSetName}">`
      );
      form.append(
        `<input type="hidden" name="is-active" value="${result.value.isActive}">`
      );
      var tables = $(".results-table");
      var total = 0;
      for (let tb of tables) {
